...
std::string FIFO::getTimeAverages(const std::vector<int>& arrival_times, const std::vector<int>& burst_times) {
	_current_process = 0;	
	...
	while (processes_unfinished) {
		...
		while (process_running) {
			...
			// or if that's the next process arriving:
				...
				_clock = arrival_times[next_process_arriving];
				_ready_queue.push(next_process_arriving);
				next_process_arriving++;
